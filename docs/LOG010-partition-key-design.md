...
- **순서는 파티션 안에서만 보장된다** (Chapter 6 복습): 토픽 전체의 순서는
  보장되지 않는다. 순서가 중요한 이벤트들은 항상 같은 파티션으로 보내야 한다.
- **파티션 키 = "어떤 단위로 순서를 보장할 것인가"**: `order-events`라면 한
  주문(`orderId`)의 이벤트 순서가 중요하지, 서로 다른 주문끼리는 순서가 섞여도
  상관없다. 키를 `orderId`로 잡는 게 자연스럽다.
- **키 선택의 트레이드오프 — Hot Partition**: 키를 쓰면 그 키의 모든 메시지가
  항상 파티션 하나로 몰린다. 트래픽이 소수의 키에 쏠리면(예: VIP 고객 하나가
  전체 주문의 대부분), 그 키가 매핑된 파티션만 과부하되고 나머지는 논다 —
  파티션을 늘려도 소용없다.
- **파티션 수를 늘리면 매핑이 깨진다** (LOG006 ADR 연결): 키의 파티션 매핑은
  해시 기반이라 파티션 수가 바뀌면 같은 키도 다른 파티션으로 갈 수 있다. 순서
  보장이 필요한 시스템에서 운영 중 파티션 수 변경은 사실상 "순서 보장 리셋".
...
컨슈머에 묶인다. 키 카디널리티가 파티션 수보다 충분히 크고 고르게 분포하면
손실이 미미하지만, 트래픽이 쏠리면 파티션을 늘려도 그 hot partition은 여전히
병목이다.

**실무 함정**: 순서 보장이 필요한 시스템에서 운영 중 파티션 수를 늘리는 것 —
해시 매핑이 깨져서 특정 시점부터 같은 키의 이벤트가 다른 파티션으로 갈 수
있다. 파티션 수는 처음 설계할 때 여유 있게 잡아야 한다.
