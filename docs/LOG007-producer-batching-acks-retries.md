...
프로듀서는 메시지를 파티션별로 메모리에 모아뒀다가(`batch.size` 또는 `linger.ms` 조건에
걸리면) 한 번에 네트워크로 보낸다. LOG006에서 짧은 시간 안에 보낸 메시지들이 한 파티션에
몰렸던 이유가 이것이다 — Sticky Partitioner가 "배치 하나 = 파티션 하나"로 묶기 때문이다.
즉 LOG006에서 "왜 그런 결과가 나왔는지"는 관찰만 하고 넘어갔던 걸, 이번 챕터에서 그
메커니즘(배치)을 정확히 짚어서 설명이 완결된다.

### 2. acks

프로듀서가 발행 성공으로 간주하는 기준이다. `0`(응답 안 기다림) / `1`(리더만 기다림) /
`all`(모든 ISR을 기다림) 세 단계가 있다. `replication-factor=1`인 지금 환경에서는 `1`과
`all`의 차이가 사실상 없다(복제할 대상이 없어서 리더 하나의 응답이 곧 ISR 전체의
응답이다) — 멀티 브로커 클러스터라야 `all`이 진짜 의미를 갖는다는 걸 실측 수치(아래
진행 과정 2번)로도 확인했다.

### 3. 재시도와 멱등성

Spring Boot는 기본으로 멱등성 프로듀서를 만든다(`enable.idempotence=true`, `acks=all`일
때). 재시도로 인한 중복을 브로커가 시퀀스 번호로 감지해서 막아준다 — 브로커가 프로듀서마다
"이 프로듀서 세션에서 마지막으로 몇 번째 시퀀스까지 저장했는지"를 기억해서, 같은 시퀀스
번호가 재시도로 다시 들어오면 중복 저장을 걸러낸다. 다만 이 보장의 범위는 "같은 프로듀서
세션 안에서의 네트워크 재시도"로 한정된다는 게 핵심이다 — 애플리케이션이 재시작되어 새
세션이 열리거나, 로직 자체가 같은 메시지를 두 번 `send()`하는 경우는 이 멱등성 범위 밖이다
(LOG017에서 이 한계를 정면으로 다룬다).
