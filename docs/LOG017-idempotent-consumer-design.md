...
### 1. Producer 멱등성의 실질적 한계

`enable.idempotence=true`는 같은 프로듀서 세션 안에서 PID(Producer ID)와
시퀀스 번호로 브로커 레벨 중복 저장을 막는다 — Ch16에서 확인한 그 범위다.
문제는 이 PID의 수명이다. 프로듀서가(트랜잭션 없이) 재시작되면 브로커는
새로운 PID를 발급한다 — 즉 "재시작 전에 이미 이 메시지를 보냈는지"에
대한 정보 자체가 재시작과 함께 사라진다. 그래서 "응답을 못 받아서 보냈는지
확신 못 하고 재전송"하는 흔한 장애 복구 시나리오(타임아웃 후 재시도)는
프로듀서 멱등성 범위 밖이다 — 새 PID로 보내는 재전송은 브로커 입장에서
완전히 새로운 메시지로 취급된다. 결론적으로 엔드투엔드 중복 방지는
프로듀서 혼자 못 하고, **컨슈머가 최종 방어선**이 되어야 한다.

### 2. Consumer 멱등성의 두 갈래

1. **멱등키 체크** — "이미 처리했는가"를 기록해두고 처리 전에 확인한다.
   이번 챕터는 이 방식을 코드로 구현했다.
2. **멱등한 연산으로 설계** — 몇 번을 실행해도 결과가 같은 연산으로
   만든다(`balance = balance + 100`처럼 이전 상태에 의존하는 상대적
   연산 대신, `balance = 100` 같은 절대값 SET이나 UPSERT로 바꾸면 같은
   요청이 여러 번 들어와도 최종 상태는 동일하다). 이 방식은 별도의 멱등키
   저장소가 필요 없다는 장점이 있지만, 모든 연산이 이렇게 "덮어쓰기" 형태로
   재설계 가능한 건 아니다(예: "재고를 1개 차감"처럼 누적이 의미 있는
   연산은 이 방식으로 바꾸기 어렵다). 도메인마다 적용 가능 여부가 달라
   이번 챕터는 1번만 코드로 다뤘다.

### 3. 원자성이 핵심

멱등키 기록과 실제 비즈니스 처리가 다른 트랜잭션에 있으면 "기록은 됐는데
처리는 안 됨"(또는 그 반대) 같은 틈이 생긴다 — 두 쓰기 사이에 프로세스가
죽으면 상태가 불일치하게 남는다. `INSERT ... ON CONFLICT DO NOTHING`처럼
체크와 기록을 원자적 연산 하나로 합치는 게 안전하다 — SELECT 후 INSERT하는
check-then-act 방식은 그 사이에 경쟁 조건 여지가 있다(두 요청이 동시에
SELECT까지는 "아직 처리 안 됨"으로 확인하고, 둘 다 INSERT까지 가버리는
레이스). `ON CONFLICT`는 DB 레벨의 UNIQUE 제약을 이용해 "먼저 도착한
하나만 성공, 나머지는 자동으로 무시"를 단일 쿼리로 해결한다.
...
가능 기간보다 짧으면 안 된다 — 업스트림이 최대 며칠까지 재전달할 수 있는
시스템이라면, 그보다 먼저 멱등키를 지워버리면 그 사이에 들어온 재전달을
"신규 이벤트"로 다시 처리해버리는 사고가 난다). 또한 이번 챕터는 단순화를
위해 `orderId`(비즈니스 키)를 그대로 멱등키로 썼는데, 실무에서는 "같은
주문에 대한 서로 다른 정당한 이벤트"(주문생성, 주문취소 등)까지 하나로
묶여 두 번째 이벤트가 막혀버릴 수 있다 — 진짜 멱등키는 이벤트마다 고유한
식별자(예: 페이로드의 UUID `eventId`)여야 하는 경우가 많다.
