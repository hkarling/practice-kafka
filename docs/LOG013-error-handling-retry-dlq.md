...
### 1. 지금까지의 한계 — 실패한 메시지가 그냥 사라진다

Chapter 8, 12에서 본 `DefaultErrorHandler`의 기본 복구 동작은 재시도를 다
소진하면 로그만 남기고 커밋을 넘겨버리는 것이었다. 실패한 메시지가 뭐였는지는
로그를 뒤지지 않는 한 사라진다 — 로그 보존 기간이 지나면 그마저도 못 찾는다.
이번 챕터는 "재시도로도 못 고치는 실패"를 조용히 버리지 않고 별도 위치에
보존하는 방법을 다룬다.

### 2. `DeadLetterPublishingRecoverer`

`DefaultErrorHandler`의 복구 단계를 커스터마이징해서, 재시도를 다 소진한
레코드를 별도 토픽(DLT, Dead Letter Topic)으로 자동 발행한다. 원본 예외
정보와 원본 위치(토픽/파티션/오프셋)를 헤더에 실어서 보내주기 때문에, DLQ만
봐도 "무엇이, 어디서, 왜" 실패했는지 추적이 가능하다 — 원본 메시지를 다시
찾으러 갈 필요가 없다.

### 3. 기본 목적지 이름 — `-dlt` 접미사

기본 DLT 토픽 이름은 `원본토픽 + "-dlt"`(하이픈, 소문자)다. Spring Kafka
4.1.0 소스를 직접 확인한 결과다 — `.DLT`(점, 대문자)가 아니다(아래 진행
과정 2번 참고, 필자가 옛날 기억으로 잘못 안내했다가 정정한 부분). 이 접미사는
리터럴 문자열로 하드코딩하는 대신 `RetryTopicConstants.DEFAULT_DLT_SUFFIX`
상수를 참조해서 조합하는 게 안전하다 — 프레임워크 버전이 바뀌어 접미사
규칙이 달라져도 코드가 자동으로 따라간다.

### 4. `ExponentialBackOff` vs `FixedBackOff`

Chapter 8의 `FixedBackOff(0, 9)`는 간격 없이 9번 연달아 재시도했다 — 일시적
장애든 영구적 장애든 구분 없이 똑같이 빠르게 두들겼다는 뜻이다. 실무에서는
보통 지수 백오프(`ExponentialBackOff`)를 써서 재시도 간격을 점점 늘린다.
여기엔 두 가지 이유가 있다 — 첫째, 일시적 장애(브로커 순간 과부하, 네트워크
지연 등)가 회복할 시간을 벌어준다. 둘째, 장애 발생 시점에 여러 컨슈머
인스턴스가 동시에 재시도를 몰아치면 오히려 장애를 악화시킬 수 있는데,
지수 백오프는 이 동시 재시도로 인한 부하 집중(retry storm)을 완화한다.
이번 챕터는 `ExponentialBackOff(500, 2.0)`에 `maxInterval=5000`,
`maxElapsedTime=15000`을 얹어, 500ms → 1s → 2s → 4s → 5s(캡) 식으로
간격을 늘리다가 총 15초가 지나면 포기하도록 구성했다.

### 5. `DefaultErrorHandler`의 기본 동작은 생성자와 무관하게 유지된다

`DefaultErrorHandler`는 `ExceptionClassifier`를 상속하는데, 그 기본
생성자가 "역직렬화 예외 등은 재시도 안 함"(Chapter 12에서 확인한 그 동작)
같은 기본 분류를 항상 설정한다. `new DefaultErrorHandler(recoverer,
backOff)`처럼 커스텀 recoverer/backOff를 줘도, 상위 클래스의 무인자
생성자가 먼저 실행되니 이 기본 분류는 그대로 살아있다 — 즉 recoverer와
backOff를 바꾸는 것과 "어떤 예외를 재시도 대상으로 볼지 분류하는 로직"은
서로 독립적인 축이라서, 하나를 커스터마이징해도 다른 하나가 리셋되지
않는다. Spring Kafka 소스로 직접 확인했다(아래 시행착오 참고).
...
"조용히 사라지는 것"이 "조용히 방치되는 것"으로 바뀔 뿐이다 — 실무에서는
DLQ 토픽에 컨슈머 랙(lag) 알림을 걸어서 "쌓이고 있다"는 신호 자체를 놓치지
않게 하는 게 최소한의 안전장치다. 알림 없이 DLQ만 만들어두면, 장애가 며칠간
방치되다가 뒤늦게 "왜 이 주문들이 하나도 처리 안 됐지"로 발견되는 식의
사고로 이어지기 쉽다.
