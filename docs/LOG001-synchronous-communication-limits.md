...
호출자가 피호출자의 주소·스키마·가용성을 전부 알아야 한다. 피호출자가 바뀌거나 잠깐
내려가면 호출자도 즉시 영향을 받는다 — 호출자 코드에는 피호출자 존재 자체가 하드코딩된
전제로 박혀 있는 셈이라, 피호출자를 교체/확장하려면 호출자도 함께 변경해야 하는 경우가
많다. 이 강결합을 물리적으로 끊어내는 게 다음 챕터(비동기 통신과 메시지 브로커)의
핵심이다 — 브로커라는 매개체를 두면 호출자는 "누가 받는지" 몰라도 된다.

### 2. 응답 대기 (Blocking)

호출자는 응답이 올 때까지 스레드를 점유한 채 기다린다. 체인이 길어질수록 지연시간이
순차적으로 누적된다 — 병렬로 처리 가능한 작업도 호출 순서가 곧 대기 순서가 되어버려서,
전체 응답 시간이 각 단계 소요시간의 **합**이 된다(병렬이었다면 **최댓값**이었을 것).
이번 챕터의 실측(아래 진행 과정 2번)이 이 합산 구조를 직접 보여준다.

### 3. 장애 전파 (Cascading Failure)

체인 하위 단계의 실패가 그대로 상위 단계까지 전파된다. Circuit Breaker 등으로 전파를
빨리 끊을 수는 있어도, 결합 자체를 없애지는 못한다 — Circuit Breaker는 "실패를 빨리
포기시키는" 도구이지 "애초에 실패가 전파되지 않게 설계를 바꾸는" 도구가 아니라는 구분이
중요하다. 이 구분은 트레이드오프/실무 함정 섹션에서 다시 짚는다.
...
상태로 남는 문제는 여전히 남는다 — Circuit Breaker는 "실패 전파를 빨리 끊는" 방어책이지 "애초에 결합을 없애는" 해법이 아니라는 걸 헷갈리면 안 된다. 이 문제는 이후 Outbox 패턴에서 다시 다룬다.
