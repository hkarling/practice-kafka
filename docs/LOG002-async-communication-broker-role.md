...
### 1. 공간적 분리 (Spatial Decoupling)

프로듀서는 컨슈머가 누구인지, 어디 있는지 몰라도 된다. 브로커에 발행만 하면 끝이다.
LOG001에서 지적한 "호출자가 피호출자의 주소·스키마·가용성을 전부 알아야 한다"는 강결합
문제를 정확히 이 지점에서 해소한다 — 호출자가 아는 대상이 "피호출자"에서 "브로커"
하나로 줄어든다.

### 2. 시간적 분리 (Temporal Decoupling)

프로듀서와 컨슈머가 동시에 살아있을 필요가 없다. 브로커가 메시지를 들고 있으니, 컨슈머가
늦게 뜨거나 잠깐 죽어도 프로듀서는 영향받지 않는다. 이건 세 가지 분리 중 가장 낯선
개념인데, 아래 진행 과정 3번에서 "컨슈머 없이 먼저 메시지를 쌓아두고 나중에 컨슈머를
띄우는" 실험으로 직접 확인한다.

### 3. 동기화 분리 (Synchronization Decoupling)

프로듀서는 발행 후 응답을 기다리지 않고 바로 리턴한다. LOG001의 "호출자가 응답이 올
때까지 스레드를 점유한 채 기다린다"는 블로킹 문제를 해소하는 지점이다.

이 세 가지가 합쳐지면 장애 전파도 자연스럽게 끊긴다 — 컨슈머 쪽 예외는 프로듀서가 이미
리턴한 뒤라 그 실행 흐름과 완전히 분리된다. 다만 이 "장애 전파 차단"은 별도로 설계한
네 번째 축이 아니라, 앞의 세 분리가 만들어낸 **부수 효과**라는 점이 중요하다 — 프로듀서와
컨슈머의 실행 흐름 자체가 이미 갈라져 있으니 한쪽의 예외가 다른 쪽 스택으로 전파될 경로
자체가 없어지는 것이다.
