...
축이다. `BATCH` ack 모드(Chapter 8)는 "언제 커밋하냐"의 문제고, 배치 리스너는
"메서드가 몇 개씩 호출되냐"의 문제다. 실제로는 자연스럽게 같이 쓰인다.
...
`source=payment-service`로 정상 수신됐다. 같은 리스너가 헤더 없이 발행된
기존 메시지(`send(key, value)`로 보낸 것들)도 같이 받는데, 그건
`source=null`로 찍혔다 — `@Header(value = "source", required = false)`
덕분에 예외 없이 넘어갔다.
...
둬야 한다. 배치 리스너를 쓸 때 `containerFactory`를 빠뜨리면 컴파일 에러도
없이 런타임에야(그것도 첫 메시지가 들어와야) `ClassCastException`으로
드러난다 — 리스너를 추가할 때마다 컨테이너 팩토리가 의도한 것과 일치하는지
확인하는 습관이 필요하다.
