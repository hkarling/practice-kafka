...
- **세 가지 배달 보장**: at-most-once(유실 가능, 중복 없음, 처리 전 커밋),
  at-least-once(유실 없음, 중복 가능, 처리 후 커밋), exactly-once(정확히
  1번, 별도 메커니즘 필요).
- **Kafka가 말하는 "exactly-once semantics(EOS)"의 실제 범위**: 공식
  문서가 말하는 EOS는 consume-transform-produce 파이프라인, 즉 **Kafka
  토픽 간 파이프라인 내부**에 한정된다. 구성 요소는 멱등성 프로듀서
  (`enable.idempotence`, Kafka 3.0+ 기본 true — 프로듀서-브로커 사이
  네트워크 재시도로 인한 브로커 레벨 중복 저장만 막음)와 트랜잭션(Ch15에서
  다룬 DB-Kafka 동기화의 그 트랜잭션 — "오프셋 커밋"과 "발행"을 원자적으로
  묶음)이다.
- **"Kafka 내부에서의 exactly-once"와 "외부 부작용까지 포함한
  exactly-once"는 다른 이야기다**: Ch15에서 이미 확인했듯, DB처럼 Kafka
  트랜잭션에 안 묶이는 외부 시스템 쓰기가 걸리면 이 보장은 깨진다. 이 갭은
  Kafka만으로는 못 메우고, 컨슈머 멱등성 설계(Ch17)나 Outbox(Ch18)가
  메우는 영역이다.
- **프로듀서 멱등성이 막아주는 것과 못 막아주는 것**: `enable.idempotence`가
  막는 건 "같은 `send()` 호출 하나가 네트워크 문제로 클라이언트 내부에서
  재시도될 때"의 브로커 레벨 중복 저장이다. 애플리케이션 코드가 **별개의
  `send()` 호출을 의도적으로 두 번** 하면, 프로듀서 입장에서는 그냥 "다른
  두 건의 정상 발행 요청"이라 전혀 못 막는다 — 이번 챕터에서 실측으로
  확인했다.
...
실측한 것처럼 그 보장은 Kafka 파이프라인 내부에만 해당된다. 또한 같은
제네릭 타입의 `KafkaTemplate` 빈이 여러 개 있는 프로젝트에서는 필드명 하나
잘못 짓는 것만으로 컴파일 에러 없이 트랜잭션 특성이 다른 프로듀서에
연결될 수 있다 — 커스텀 Kafka 빈이 누적될수록 이 위험도 커진다.
