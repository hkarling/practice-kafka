...
### 1. Point-to-Point (P2P, 큐 모델)

메시지 하나는 여러 컨슈머 중 **정확히 하나**만 가져간다. 컨슈머를 늘리는 목적은 처리량
분산 — 워커 풀(worker pool) 패턴과 동일하다. 이미 LOG002의 `OrderConsumer`/`QueueManager`
구조가 컨슈머 하나짜리 P2P였다 — 이번 챕터는 그 컨슈머를 여러 개로 늘렸을 때도 "메시지가
정확히 한 번씩만 나뉘어 처리되는가"를 확인하는 확장이다.

### 2. Pub/Sub (발행-구독 모델)

메시지 하나를 **구독 중인 모든 컨슈머가 각자 전부** 받는다. 컨슈머를 늘리는 목적은 처리량
분산이 아니라, 같은 이벤트에 서로 다른 관심사를 가진 소비자들에게 전부 알리기 위해서다 —
예를 들어 "주문 생성" 이벤트 하나를 재고 차감 서비스와 알림 발송 서비스가 각자 독립적으로
받아서 서로 다른 일을 해야 하는 상황이다.

### 3. 두 모델의 목적과 구현 방식 비교

| | Point-to-Point | Pub/Sub |
|---|---|---|
| 메시지 하나를 받는 컨슈머 수 | 정확히 1개 | 구독자 전원 |
| 컨슈머를 늘리는 목적 | 처리량 분산 (워커 풀) | 서로 다른 관심사에 각자 알림 |
| 대응하는 이번 챕터 구조 | 공유 큐(`QueueManager`) | 구독자별 전용 큐(`PublishManager`) |
| 처리량을 늘리는 법 | 컨슈머(워커) 수를 늘린다 | 각 구독자 **내부**에서 워커를 늘린다 |

### 4. Kafka는 이 둘을 Consumer Group으로 동시에 구현한다

같은 그룹에 속한 컨슈머들끼리는 P2P처럼 파티션을 나눠 갖고, 서로 다른 그룹은 Pub/Sub처럼
각자 토픽 전체를 독립적으로 구독한다. 이 이중성이 Kafka가 "메시지 큐"이자 "이벤트
스트림" 둘 다로 불리는 이유다(LOG004에서 더 다룬다). 실무에서 이 둘을 조합하는 흔한
패턴이 위 표의 마지막 줄이다 — "Pub/Sub 토픽을 여러 서비스가 각자 구독하되, 각 서비스는
자기 안에서 Consumer Group으로 워커를 늘려 P2P 처리량 분산까지 얻는" 구조다.
